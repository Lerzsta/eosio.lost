...
const KeypalConnect = require('../keypal-connect').default;
...
const keyringType = 'Keypal Hardware';
const pathBase = 'm';
const MAX_INDEX = 1000;
const DELAY_BETWEEN_POPUPS = 1000;
const KEYPAL_CONNECT_MANIFEST = {
  email: 'support@tp.io',
  appUrl: 'https://tp.io',
};
...
class KeypalKeyring extends EventEmitter {
  constructor(opts = {}) {
    super();
    ...
    KeypalConnect.on('DEVICE_EVENT', (event) => {
      if (event && event.payload && event.payload.features) {
        this.model = event.payload.features.model;
      }
    });
    KeypalConnect.init({ manifest: KEYPAL_CONNECT_MANIFEST });
  }
      ...
      KeypalConnect.getPublicKey({
        ...
        .then((response) => {
          if (response.success) {
            ...
            this.deviceName = response.payload.deviceName;
            KeypalKeyring.deviceName = this.deviceName
  ...
  async _signTransaction(address, chainId, tx, handleSigning) {
    ...
    try {
      ...
      const response = await KeypalConnect.ethereumSignTransaction({
    ...
    return new Promise((resolve, reject) => {
      this.unlock()
        .then((status) => {
          setTimeout(
            (_) => {
              KeypalConnect.ethereumSignMessage({
...
KeypalKeyring.type = keyringType;
module.exports = KeypalKeyring;
